...
export default function Kitties(props) {
  ...
  useEffect(() => {
    const fetchKittyIndexes = async () => {
      const kittyIndex = (
        await api.query.kitties02Module.nextKittyId()
      ).toNumber();
      if (kittyIndex <= 0) {
        return;
      }
  ...
  useEffect(() => {
    ...
    const fetchKitties = async () => {
      const owners = await api.query.kitties02Module.kittyOwner.multi(
        kittyIndexes
      );
      const kittyDNAs = await api.query.kitties02Module.kitties.multi(
        kittyIndexes
      );
